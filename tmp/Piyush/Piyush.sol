 ...
 */
contract PiyushToken is Ownable, MintableToken {
  ...
  // Token details
  string public constant name = "PiyushToken";
  string public constant symbol = "PIYU";
  ...
  // to get exponent
  function getExponent() public returns (uint256 _exp) {
     _exp = exponent;
  }

  // to get minted
  function getTokensMinted() public returns (uint256 _minted) {
     _minted = tokensMinted;
  }

  // to get totalSupply
  function getTotalSupply() public returns (uint256 _total) {
     _total = totalSupply;
  }
   ...
   */
  function PiyushToken(address _owner) {
      //Total of tokens
      totalSupply = 1122 * exponent;
      //mint(TEAM_ADDRESS, undefined * exponent);
      //mint(ADVISOR_ADDRESS, undefined * exponent);

      // tokens for Funders+advisors, PreSales
      // tokensMinted = tokensMinted.add(undefined * exponent);
      // balances[TEAM_ADDRESS] = undefined * exponent;
      // tokensMinted = tokensMinted.add(undefined * exponent);
      // balances[ADVISOR_ADDRESS] = undefined * exponent;
  }
  ...
  function batchTransfers(address[] _accounts, uint256[] _tokens) onlyOwner public returns (bool) {
    ...
    for (uint i = 0; i < _accounts.length; i++) {
      ...
      require(_tokens[i] > 0);
      transfer(_accounts[i], _tokens[i] * exponent);
  ...
  function raiseInitialSupply(uint256 _supply) onlyOwner public returns (bool) {
      totalSupply = totalSupply.add(_supply * exponent);
      return true;
  }

  /**
   @dev Burns a specific amount of tokens.Token that can be irreversibly burned (destroyed).
...
/**
 * @title Piyush CrowSale/ICO contract
 ...
 */
contract PiyushCrowdsale is Ownable
{
    using SafeMath for uint256;

    // The token being sold
    PiyushToken public token;
     ...
     */
    function PiyushCrowdsale() {
        wallet = msg.sender;
        token = new PiyushToken(msg.sender);
    }

    /**
      @dev function to start the crowdsale with predefined timeslots and discounts. it will be called once for every crowdsale session and it can be called only its owner
    ...
    /**
      @dev proxy method for Piyush Tokens batch transfers method, so that contract owner can call token methods
      @param _accounts buyers accounts that will receive the presale tokens
      @param _tokens   Amount of the tokens to be transferred to each account in _accounts list
      @return A boolean that indicates if the operation is successful.
     */
    ...
    /**
      @dev proxy method for Piyush Tokens raiseInitialSupply method, so that contract owner can call token methods
      @param _supply delta number of tokens to be added to total supply
      @return A boolean that indicates if the operation is successful.
     */
    function raiseInitialSupply(uint256 _supply) onlyOwner public returns (bool) {
    ...
    /**
      @dev proxy method for Piyush Tokens burn method, so that contract owner can call token methods
      @return A boolean that indicates if the operation is successful.
     */
    function burnLeftOverTokens() inactiveCrowdSale onlyOwner public returns (bool) {
        uint256 leftOverTokens = token.getTotalSupply().sub(token.getTokensMinted());
        token.burnToken(leftOverTokens);
        return true;
    }
    ...
    function buyTokens(address beneficiary) activeCrowdSale public payable {
        ...
        require(numTokens > 0);
        var numTokensWD = numTokens.mul(token.getExponent());
        tokensMinted = tokensMinted.add(numTokensWD);

        // Mint the tokens and trasfer to the buyer
        token.mint(beneficiary, numTokensWD);
